                ...
                <div class="row g-4">
                        ...
                        <select class="form-select mb-3" aria-label="Default select example" name="owner">
                            ...
                            <c:forEach var="member" items="${teamMember}">
                                <option value="${member.id}" 
                                        <c:if test="${member.id == ownerID_s}">selected</c:if> >
                                    ${member.userName}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <c:if test="${requestScope.teamRole == 9}">
                        <div class="col-sm-12 col-xl-1">
                            <button type="button" class="btn btn-success " onclick="window.location.href = '${pageContext.request.contextPath}/requirement?action=Add&teamID=${requestScope.teamID}&tab=team&classID=${sessionScope.classID}'">Add</button>
                        </div>
                    </c:if>
